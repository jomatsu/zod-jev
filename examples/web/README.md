...
出品画面の「下書き一覧」を開くと、7 件の下書きから**ワンクリックで入力**できます。押した瞬間にフォームとプレビューが入れ替わり、
「出品する」を押すと結果（公開中 / 審査中 / 出品できません＋理由）が返ります。
...
- **初期表示は「前回の下書き」** として用意しています（実際のアプリでも下書きは保存される）。上部の「下書き一覧」から**ワンクリックで他の下書き（弾かれる例を含む）に切り替え**られます。中身はサーバーの `/api/meta`（`fake.ts` の `samples`）が唯一の定義で、`/ops` のデモボタンと同じデータを使っています。
